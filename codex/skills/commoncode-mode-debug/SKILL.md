---
name: commoncode-mode-debug
description: MANDATORY MODE for systematic diagnostics and bug fixing. Focuses on root-cause identification through LDD trace analysis and code immunization. Loaded by the root Orchestrator when the QA or Code phases return failures.
---
...
- MANDATORY: do a local file read of ALL modules mentioned in the traceback and their dependencies.
- **Dependency discovery order (cheap → expensive):**
  1. **GREP-scan for instant overview.** Run a terminal grep with pattern `GREP_SUMMARY|STRUCTURE` across the target project's source folders. The 1–2 line summary per file (`# GREP_SUMMARY:` keywords + `# STRUCTURE:` mini block diagram) reveals which files are relevant before any full read.
  2. **Doxygen XML index.** If `<target>/doxygen_output/xml/` exists (it does after `commoncode-mode-code` Step 7 — `BUILD_DOXYGEN`), read the relevant per-file XML reports (e.g., `mymodule_8py.xml` for `mymodule.py`) to enumerate callers, callees, and cross-links. If the XML is stale or missing, run terminal `cd <target> && doxygen Doxyfile` to regenerate (Doxygen is fast — this is safe at any time).
  3. **Fallback to source.** Use terminal grep / glob over `<target>` for symbols, then do a local file read of the relevant files. Do **not** look for `AppGraph.xml` — that artifact has been deprecated; the post-code architecture lives inside Doxygen output and the in-source `## @modulemap` / `## @links` / `# region` markup.
...
- Apply the fix via a patch (local file write).
...
- **Do not perform direct fixes for large changes** — prefer re-entering the Code phase (load `commoncode-mode-code` skill) with a fresh framing, re-anchoring only on the affected file + the Bug Report. A direct surgical one-line patch from the root is acceptable for trivial fixes.

**Step 4 — SEMANTIC_VERIFICATION**
- Run terminal `python -m pytest <tests> -s -v`.
- MANDATORY: audit new logs (IMP:7-10) to verify the logic. Do not trust green asserts alone.
- Optionally switch into the QA phase (load `commoncode-mode-qa` skill) for impartial re-verification with a fresh framing.
