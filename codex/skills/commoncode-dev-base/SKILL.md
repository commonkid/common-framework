...
Скиллы `commoncode-mode-architect` / `commoncode-mode-code` / `commoncode-mode-qa` описывают **процесс**. Этот скилл описывает
...
1. `commoncode-mode-architect` → `DevelopmentPlan.md` с секциями **Draft Code Graph** и **Data Flow**
...
2. `commoncode-mode-code` → реализация с экзоскелетом, LDD и тестами.
3. `commoncode-mode-qa` → независимый прогон pytest + Diagnostic Trio (Logs / Code / Data).
