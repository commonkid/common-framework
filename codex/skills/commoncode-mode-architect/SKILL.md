---
name: commoncode-mode-architect
description: MANDATORY MODE for project design and planning. Must be invoked before any code modification to create a Development Plan, run superposition of hypotheses, and coordinate the Code/QA phases. Loaded by the root Orchestrator.
---

# Mode: Architect — Main Workflow

Your primary goal in this mode is **not to write code**, but to **design** it. You act as a system analyst and software architect. Your goal is to explore the solution space, avoid local optima, and create robust plans for the implementation phases.

In the Codex port, **you (the root Orchestrator) execute this playbook directly** because Human-in-the-Loop gates require natural dialog with the User. The flow runs in a single context: after the plan is approved, you switch into the Code phase by loading the `commoncode-mode-code` skill and executing it yourself, then into the QA phase via the `commoncode-mode-qa` skill. Debugging loops back to you via the `commoncode-mode-debug` skill.
...
- **Framework host** = your current working directory (contains `.codex/`). This directory is **READ-ONLY** for the duration of the flow. It hosts `prompts/`, `skills/`, and potentially legacy artifacts from prior work. You MUST NOT write, edit, or create files here during the flow.
- **Target project** = the absolute path passed as `$1` to the develop prompt. ALL produced artifacts (`DevelopmentPlan.md`, `business_requirements.md`, source code, tests, logs, Doxygen output under `doxygen_output/xml/`) are written under this path.
...
- **Targeted Refactoring / Feature Addition:** improving or extending existing code. **Your first mandatory step in this case is to perform a GREP-scan** for `GREP_SUMMARY|STRUCTURE` across the target source folders for an instant per-file overview, then read the Doxygen XML reports under `<$1>/doxygen_output/xml/` (run terminal `cd "$1" && doxygen Doxyfile` to refresh if missing or stale) to understand the current architecture, cross-links, and dependencies.
...
- **Actions:**
  0. **Study implementer capabilities.** Before starting the design, study the rules of the Code and Debug modes:
     - Do a local file read of `~/.codex/skills/commoncode-mode-code/SKILL.md` (the full playbook of the Code phase — its constraints and capabilities).
     - Load the `commoncode-mode-debug` skill (read `~/.codex/skills/commoncode-mode-debug/SKILL.md`) to understand the debug protocol you may need later.
     - This is necessary to understand the limitations and capabilities of the phases through which you will run implementation.
  1. **[HITL GATE 1]** Ask the User (in Russian, as a plain question to the user) about their ultimate intentions. Pause for response. Do not guess.
  ...
  3. **Extract task-specific conventions and constraints.** Parse `$2` (the task file) for:
     - Naming / numbering conventions (e.g., "next sequential lesson number", "feature branch naming", "module prefix rules"). Compute concrete values NOW (via glob / terminal `ls` inside `$1`) so the downstream Code phase receives concrete names, not rules-to-apply.
     ...
     Record all of these — they MUST be re-anchored when you enter the Code phase at Step 5.
...
- **Actions:**
  ...
  2. For unknown libraries, use context-search (e.g., web search, or Context7 MCP if available).
...
- **MANDATORY:** You MUST load the `commoncode-devplan-protocol` skill (read `~/.codex/skills/commoncode-devplan-protocol/SKILL.md`) AND the `commoncode-document-protocol` skill (read `~/.codex/skills/commoncode-document-protocol/SKILL.md`) to ensure compliance with mandatory structural protocols. Failure to use these protocols will produce artifacts that are uninterpretable for the agent swarm.
...
- **[HITL GATE 3]** Wait for the User's approval of the comprehensive plan before entering the Code phase. Pause for response.

### Step 5 — `DELEGATE_IMPLEMENTATION` (Running the Code Phase)

- Switch into the Code phase by loading the `commoncode-mode-code` skill (read `~/.codex/skills/commoncode-mode-code/SKILL.md`) and execute its playbook yourself. Because the flow runs in a single context, you carry the design forward directly — but you MUST re-anchor on the on-disk artifacts and constraints below before writing any code, treating them as the single source of truth.
- **Required context to re-anchor before entering the Code phase** (ALL ARE MANDATORY):
  1. **Target project absolute path** (`$1`). Reaffirm: *"Work exclusively inside this directory. The framework host is read-only."*
  ...
  5. Explicit scope of this slice (Feature Slice) including tests.
  ...
  9. Reminder to yourself: *"Follow the Code playbook step-by-step. On completion, produce either SUCCESS + artifact paths, or a structured Bug Report."*
- **CRITICAL RULE (Feature-Complete):** Implement a **complete functional slice along with its tests** in one pass. It is forbidden to separate code and tests into different passes.
- **Anti-Loop Discipline:** If you cannot solve the problem in 2–3 iterations, stop and produce a **Bug Report** using the established template (Logs + Code + Data), then switch into the Debug phase with a fresh framing.

### Step 6 — `SWARM_VERIFICATION & DEBUG` (Acceptance and Debugging)

- After the Code phase finishes, switch into the QA phase by loading the `commoncode-mode-qa` skill (read `~/.codex/skills/commoncode-mode-qa/SKILL.md`) for **Extended Diagnostics (QA)** if necessary. Re-anchor on the absolute path to `tests/test_guide.md`.
- If tests fail, analyze the Bug Report produced by the QA phase.
- **Load the `commoncode-mode-debug` skill** (read `~/.codex/skills/commoncode-mode-debug/SKILL.md`) and follow its Diagnostic Playbook, then re-enter the Code phase with a **fresh framing** — re-anchoring only on the specific code and the essence of the report. This excludes "context fatigue" and looping on old errors.
...
- **Development Plan template (`commoncode-devplan-protocol` skill):**
...
- **General document template (`commoncode-document-protocol` skill):**
...
- For data-transformation tasks (ETL, Pandas, SQL), you MUST load the `commoncode-data-transform` skill (read `~/.codex/skills/commoncode-data-transform/SKILL.md`) before planning.

---

## 6. Final Review of Completed Work

After finishing development through the Code and QA phases, perform a final review:
...
3. **Doxygen build (post-code architecture index):** verify or create `<$1>/Doxyfile` (with `INPUT` covering target source folders, `EXCLUDE` for venv / `node_modules` / `doxygen_output`), then run terminal `cd "$1" && doxygen Doxyfile`. Confirm per-file XML reports exist under `<$1>/doxygen_output/xml/`. If Doxygen reports inline-documentation syntax errors in `## @…` tags, re-enter the Code phase for a fix with the offending file + error excerpt, then re-run.
...
5. If other deficiencies are found, re-enter the Code phase to fix them with a fresh framing.
