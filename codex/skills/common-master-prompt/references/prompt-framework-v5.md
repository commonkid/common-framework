...
> Целевая модель: **GPT-5 Codex** (через Commoncode / Codex).
...
1. `[project]-MASTER-INSTRUCTION.md` — мастер-инструкция для Codex
2. `AppGraph.xml` — навигационный граф архитектуры

**Плюс при выполнении:**
3. `/SprintLog/Sprint-NNN-[название].md` — создаётся Codex после завершения спринта
...
Codex при работе с промптом фазы:
...
После завершения каждого спринта Codex записывает туда подробный
MD-файл с описанием всего, что было сделано.

Цель:
- В будущем Codex может ходить в эти логи
...
Codex **обязан** создавать файл строго по этому шаблону:
...
Для **каждой фазы** Codex обязан зафиксировать:
...
Сводная таблица всех спринтов (обновляется Codex при создании нового файла):
...
### 5.7. Правило для Codex: навигация через SprintLog

Когда Codex работает с промптом фазы и видит в теле:
...
Codex **НЕ УДАЛЯЕТ и НЕ МОДИФИЦИРУЕТ** их, **за исключением блока
...
Цель Фазы 0: агент (GPT-5 Codex) полностью анализирует существующий
...
phase_number: 0
date: YYYY-MM-DD
author: AI (GPT-5 Codex) — generator
...
агент (GPT-5 Codex) **обязан** провести короткий интерактивный
...
где Codex будет его потом искать.
...
## 14. Оптимизация под GPT-5 Codex

### 14.1. Особенности модели

GPT-5 Codex — наиболее мощная модель в семействе Codex 4.6.
...
└──────────────────────┬───────────────────────────────┘
                       ...
                       ▼
┌──────────────────────────────────────────────────────┐
│  4. Оператор передаёт файлы агенту (GPT-5 Codex)│
...
- После каждой фазы Codex дописывает раздел в `Sprint-007-notifications.md`
