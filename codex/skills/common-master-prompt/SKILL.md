...
* **Политика сохранности фреймворка:** ни при каких условиях не изменять содержимое `references/prompt-framework-v5.md` из папки скилла. Этот файл — read-only источник истины.

# END_INTERACTION_PROTOCOL

# START_WORKFLOW_STEP_1_INTAKE

**Первым действием при запуске скилла** агент задаёт оператору ЕДИНСТВЕННОЕ сообщение с 3 пронумерованными вопросами (через plain Russian text или обычный текст — по обстоятельствам):
...
После подтверждения оператора — **прочитать целиком** файл `<SKILL_DIR>/references/prompt-framework-v5.md` через Read (все 2053 строки, без фрагментирования). Это sovereign source of truth. При любом расхождении между этим SKILL.md и фреймворком — приоритет у фреймворка.
...
6. Нельзя удалять слова из `references/prompt-framework-v5.md` внутри папки скилла — это zero-modification reference.
...
* **Sovereign framework:** `<SKILL_DIR>/references/prompt-framework-v5.md` (читать целиком перед генерацией).
