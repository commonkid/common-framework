...
**Scope (read first):**
This protocol applies to **pre-code design artifacts** only — the `DraftCodeGraph` block inside `DevelopmentPlan.md` produced by `mode-architect`. At that moment the code does not yet exist, so Doxygen cannot index it; an explicit XML graph is the Architect's thinking tool.

**Out of scope (deprecated):** producing a post-code `AppGraph.xml` to summarize an already-implemented codebase. That role is replaced by Doxygen-generated XML reports under `<target>/doxygen_output/xml/` (created by `BUILD_DOXYGEN` in `mode-code` Step 7). Do not write any `AppGraph.xml` artifact on disk during the flow.

**Objective:**
Provide a strict, parseable XML format for the Conceptual Code Graph the Architect drafts in advance — so subsequent agents (`mode-code`, `mode-qa`) can read the design intent before any source file exists.
...
|-------|----------|----------|----------|
| Pre-code design | `<DraftCodeGraph>` block inside `DevelopmentPlan.md` | `mode-architect` (Step 4 — DESIGN_AND_VALIDATE_SOLUTION) | `mode-code`, `mode-qa` |
| Post-code architecture index | Doxygen XML at `<target>/doxygen_output/xml/` | `mode-code` (Step 7 — BUILD_DOXYGEN, runs `doxygen Doxyfile` inside `<target>`) | All agents (navigation, dependency discovery) |
