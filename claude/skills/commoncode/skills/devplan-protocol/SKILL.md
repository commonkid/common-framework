...
description: CORE PROTOCOL for all development planning. Implementation agents will reject plans that do not strictly follow this schema. Mandatory for all development tasks — loaded by mode-architect during Step 4 (DESIGN_AND_VALIDATE_SOLUTION).
---
...
**MANDATORY:** This block MUST be in XML format using the rules from the `graph-protocol` skill (invoke `Skill` tool with `name: commoncode:graph-protocol` if you need a refresher on the formatting standard).

**Purpose of this block:** the `DraftCodeGraph` is the Architect's **thinking tool** for the moment when the code does not yet exist. It declares structural anchors and cross-links *before* implementation, so `mode-code` and `mode-qa` can read design intent from the plan alone. It is **not** a snapshot of an already-implemented codebase — post-code navigation and dependency discovery use Doxygen XML (`<target>/doxygen_output/xml/`, generated by `mode-code` Step 7 — `BUILD_DOXYGEN`). Do not produce or update any standalone `AppGraph.xml` artifact during the flow.
