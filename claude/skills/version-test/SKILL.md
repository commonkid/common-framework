...
argument-hint: "[status | bump-major | bump-minor | init vX.Y.Za | set vX.Y.Za]"
user-invocable: true
---
...
**If build or tests fail — automatically launches KiloCode agents to diagnose and fix.**
...
Build and test checks (5-6) trigger the **KiloCode Auto-Fix Pipeline** on failure (see Step 3.1).
...
```
- PASS: exit code 0
- FAIL: capture full error output → **proceed to Step 3.1 (KiloCode Auto-Fix)**
...
### Step 3.1 — KiloCode Auto-Fix Pipeline (on Build/Test failure)

When Check 5 or Check 6 fails, **do NOT simply stop**. Instead, ask the user:

```
⚠️  Build/Tests failed. Launch KiloCode agents to diagnose and fix?
```
...
| Option | Action |
|--------|--------|
| **Auto-fix with KiloCode (Recommended)** | Launch full KiloCode debug → fix → verify pipeline (see below) |
| **Fix manually** | Show errors, STOP. User fixes on their own and re-runs `/version-test` |

**If user chooses Auto-fix — execute the KiloCode pipeline:**

**Phase 1: DEBUG — Root cause analysis**

Invoke `Skill(skill='kilocode:mode-debug')` with the build/test error output as context.
...
Invoke `Skill(skill='kilocode:mode-code')` with the Bug Report as input.

The code agent will:
1. Apply fixes following KiloCode semantic markup:
...
| Attempt | Action |
|---------|--------|
| 1 | Re-run `kilocode:mode-debug` with new error output. Apply fix. Re-verify. |
| 2 | Load `kilocode:mode-debug` with accumulated context from attempts 1-2. Apply fix. Re-verify. |
...
**Anti-Loop protection:** Track attempts via `.test_counter.json` (KiloCode convention):
```json
...
If `tests/test_guide.md` exists after fixes, invoke `Skill(skill='kilocode:mode-qa')` for independent verification:
