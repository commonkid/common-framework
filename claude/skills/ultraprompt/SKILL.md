...
argument-hint: "<project/feature description> | run | v1.2.3a | status"
user-invocable: true
---
...
2. **Read current product version:**
   ```bash
...
Read current version from VERSION file and assign sequential indices:
...
**Every generated prompt-contract MUST include this block.** It is the bridge between WHAT to build (the prompt-contract) and HOW to write the code (KiloCode semantic markup). Without it, the executing agent has no structural expectations to uphold.
...
**KiloCode — Architecture tasks (v*.*.1a):**
...
**KiloCode — Implementation tasks (all others):**
- [ ] FORMAT lists files with MODULE_MAP complexity estimates?
- [ ] FORMAT includes AAG Use Cases (Actor → Action → Goal)?
- [ ] CRITERIA includes mandatory KiloCode checks (LDD, MODULE_CONTRACT, Anti-Loop)?
...
**3.2 Ask user how to execute this task** via `AskUserQuestion`:

| Option | Action |
|--------|--------|
| **KiloCode Pipeline** | Load `Skill(skill='kilocode:mode-architect')`, then `mode-code`. Full KiloCode semantic markup, SFT-priming, LDD logging. |
| **Direct Development** | Execute the prompt-contract directly in current Claude Code session. Write code, create files, no KiloCode overhead. |
...
- If **KiloCode Pipeline**: invoke `Skill(skill='kilocode:mode-architect')` with the prompt-contract as context, then proceed through the KiloCode workflow
...
**3.4 Post-task decision** via `AskUserQuestion`:
...
| **Test & Debug** | Enter testing mode: user tests the functionality, reports bugs. For each bug create a new Issue with label `bug, vX.Y`. If user wants AI to fix — load `Skill(skill='kilocode:mode-debug')` or fix directly. Loop until user says "done testing", then close task and continue. |
...
Invoke `Skill(skill='version-test')` which runs 6 checks:
...
- Next step: "Ready for next milestone. Run `/ultraprompt <next feature>` → tasks will start at v1.4.1a"

---

## MODE C: SINGLE TASK

Parse the version from `$ARGUMENTS` (e.g., `v1.1.3a`).
...
KiloCode-mandatory (ALWAYS include for implementation tasks):
...
KiloCode-mandatory (ALWAYS include for architecture tasks):
- "DevelopmentPlan.md contains: Draft Code Graph (XML), Data Flow, Acceptance Criteria"
- "AppGraph.xml updated with new modules and CrossLinks"]
$END_CRITERIA
