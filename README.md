...
* **Статуслайн** — проект и ветка, тариф, модель и уровень рассуждения слева (в режиме
  ...
  `statusline.json`); контекст справа. Третьей строкой, без рамки и разделителей, идут цена и время:
  **`session`** — стоимость этого чата, **`t`** — сколько этот чат уже идёт (часы, минуты и секунды,
  по `cost.total_duration_ms` от Claude Code). Если идёт деплой через `pipeline`,
